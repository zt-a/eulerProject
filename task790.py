...
Создадим последовательность $S_t$, где:
$$
\begin{align}
S_0 &= 290797\\
...
\end{align}
$$
Четыре числа $N_t = (S_{4t-4}, S_{4t-3}, S_{4t-2}, S_{4t-1})$ описывают область сетки, где первая пара чисел представляет границы области по оси $x$, а вторая пара чисел - границы по оси $y$. Например, при $N_t = (3,9,47,20)$, область ограничена $3\le x\le 9$ и $20\le y\le47$ и содержит 196 часов.
